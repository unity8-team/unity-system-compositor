...
DBusHandlerResult usc::UnityScreenService::handle_dbus_message(
    ::DBusConnection* connection, DBusMessage* message_raw, void* user_data)
{
    DBusMessageHandle message{UnownedDBusMessage{message_raw}};
    ...
    else if (dbus_message_is_method_call(message, dbus_screen_interface, "setUserBrightness"))
    {
        ...
        if (!args_error)
        {
            log::unity_screen_service_request(message, {"brightness"});
    ...
    else if (dbus_message_is_method_call(message, dbus_screen_interface, "userAutobrightnessEnable"))
    {
        ...
        if (!args_error)
        {
            log::unity_screen_service_request(message, {"enable"});
    ...
    else if (dbus_message_is_method_call(message, dbus_screen_interface, "setInactivityTimeouts"))
    {
        ...
        if (!args_error)
        {
            log::unity_screen_service_request(message, {"poweroff_timeout", "dimmer_timeout"});
    ...
    else if (dbus_message_is_method_call(message, dbus_screen_interface, "setScreenPowerMode"))
    {
        ...
        if (!args_error)
        {
            log::unity_screen_service_request(message, {"mode", "power_state_change_reason"});
            ...
            DBusMessageHandle reply{
                ...
                DBUS_TYPE_INVALID};
            log::unity_screen_service_reply(reply, {"result"});
    ...
    {
        log::unity_screen_service_request(message);
        ...
        DBusMessageHandle reply{
            ...
            DBUS_TYPE_INT32, &id,
            DBUS_TYPE_INVALID};
        log::unity_screen_service_reply(reply, {"id"});
    ...
    else if (dbus_message_is_method_call(message, dbus_screen_interface, "removeDisplayOnRequest"))
    {
        ...
        log::unity_screen_service_request(message, {"id"});
    ...
    if (args_error)
    {
        ...
        log::unity_screen_service_reply(reply, {"error"});
    ...
    std::string const& new_owner)
{
    if (new_owner.empty() && old_owner == name)
    {
        ...
        if (keep_display_on_ids.find(name) != keep_display_on_ids.end())
            log::unity_screen_service_peer_disappearance(name);
